...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $idproprietario = filter_input(INPUT_POST, 'Proprietarios_idProprietarios');

    if(empty($id) || empty($nome) || empty($distrito) || empty($concelho) || empty($freguesia) || empty($artigo) || empty($cultura) || empty($idproprietario)) {
...
$sql = "UPDATE propriedadesagricolas SET Nome='$nome', Distrito='$distrito', Concelho='$concelho',Freguesia='$freguesia',artigoMatricial ='$artigo',Culturas='$cultura',Proprietarios_idProprietarios='$idproprietario' WHERE idPropriedades= '$id'";
